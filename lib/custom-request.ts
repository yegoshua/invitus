...
export const customRequestSchema = customRequestFormSchema.extend({
  ...
  artwork: z.object({
    pathname: z
      .string()
      .max(300)
      // One flat folder, no climbing out of it: the endpoint reads this file
      // with the store's own credentials and posts it to the team's chat.
      .regex(/^custom-belt\/[A-Za-z0-9._-]+$/, "Невідомий файл")
      .refine((p) => !p.includes(".."), "Невідомий файл"),
