...
- **Presentation content is hybrid**: KeyCRM has no marketing content — 3D models, hero/bg images, howToMeasure, careInstructions and `featured` come from **Strapi** via `lib/product-extras.ts`, matched by the optional `keycrmId` number field on the Strapi product (add in Content-Type Builder; rename-proof) or by slug (Strapi slug ↔ slugified KeyCRM name). Resilient: 4s timeout, single attempt, 60s back-off — a down Strapi degrades to fallbacks and never blocks rendering. `content/product-extras.ts` (keyed by KeyCRM product id) is the offline fallback layer. Filter tags are not wired up (catalog shows only "УСІ").
